...
class OhemCrossEntropyLoss(nn.Layer):
    ...
    def forward(self, logit, label):
        ...
        #     if i == 255:
        #         lbmax += 1
        # lbsum 0  lbmax 741425 推测label读取问题？
        # print(lbsum)
        # print(lbmax)

        ######### 待解决！
        # if type(self.ignore_index) == list:
        #     valid_mask = (label != self.ignore_index[0] and label != self.ignore_index[1]).astype('int64')
        # else:
        
        if type(self.ignore_index) == list:
            valid_mask = (label != self.ignore_index[0])
            valid_mask = (valid_mask != self.ignore_index[1]).astype('int64')
        else:
